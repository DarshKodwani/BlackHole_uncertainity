...
More precisely, the particular effect of local geometric uncertainty here is to create extra shifts between parallel null geodesics, which is reminiscent of memory effects or supertraslation\cite{GW_memory,HeLys14,KodPen16}.
This naturally echoes the idea that the soft hairs \cite{HawPer16} encodes the black hole information and returns them to the Hawking radiation via an interaction that is beyond the na\"ive local QFT\cite{OsuPag16}. 
