...
\title{A self-consistency check for unitary Hawking radiation}
...
The change of distance (arrival time difference) between these two geodesics qualitatively represent the change a mode function will have.
As such change gets large, we should lose faith on the apparent unitarity in the QFT calculation.
...
In our case, we will see that even in Minkowski space, geometric uncertainty already leads to an in-principle-finite change between the two null geodesics.
...
Our surprising result is that in the black hole case, we get an arbitrarily larger effects than in Minkowski space, which is difficult to blame on the usual divergence of QFT.
\end{itemize}
...
We then introduce a geometric uncertainty as two shells of zero total ADM mass. 
...
\begin{equation}
	\frac{\delta t_0 - \bar{ \delta t_0}}{\delta t_0} = \frac{\Delta M \Delta r}{(r_1-2M)^2}	\label{lim-result}
\end{equation}

This expression can be evaluated in three limits; 1) The Minkowski limit ($M=0$), 2) Far from horizon ($M <<r_1$), 3) Near horizon limit ($r_1<<M$). These limits have to be calculated using physical, local quantities, which are the local surface energy of the shell $\sigma$ and the proper distances. The calculations are performed in appendix \ref{finalEq} and the results are summarized below:
...
We thank XXX for discussions. We are supported by XXX fundings.
...
\section{Deriving Eq (\ref{fin-result})}\label{finalEq}
\label{finalEq}
...
%As the first step, we set $M=0$ in Eq.~(\ref{lim-result}) to study its meaning in Minkowski space,
%
%\begin{equation}
...
%\end{equation}
%
%Next we study Eq.~(\ref{lim-result}) with an actual black hole. 
%Before we perform any analysis with a black hole it is important to convert the coordinate quantities in Eq (\ref{lim-result}) to physical quantities. We implicitly did this in the Minkowski limit; the variables in Eq (\ref{eq-Mink}) indeed represent the physical surface energy of the shell and the proper distance to it. In the case of a black hole we have to be more careful. The natural quantity to use for the local surface energy of the shell is the time component of the surface stress energy term, $S_{ab}$, given by the IJC
...
\begin{equation}
	\frac{\delta t_0 - \bar{t_0}}{\delta t_0} = \frac{\Delta M \Delta l}{l^2} = \sigma \Delta l.
\end{equation}

\subsection{Far from horizon limit ($M<<r_1$)}

In this limit Eq (\ref{l-r}) can be expanded to first order to give
...
\begin{eqnarray}
	\frac{\delta t_0 - \delta \bar{t_0}}{\delta t_0} & = & 4 \pi \sigma \frac{r^2}{(r_1 - 2M)^2} \left( 1 - \frac{2M}{r_1} \right)^{\frac{1}{2}} \left( 1 - \frac{2M}{r_1} \right)^\frac{1}{2}  \nonumber	\\
	& = & 4 \pi \sigma \Delta l \left( 1  + \frac{2M}{r_1 - 2M} \right)^2 \left( 1 - \frac{2M}{r_1} \right)	\nonumber	\\
	& = & 4 \pi \sigma \Delta l \left( 1 + \frac{2M}{l} \right) + \mathcal{O}(l^{-2})
\end{eqnarray}
...
\begin{eqnarray}
	...
	& = & 64 \pi \sigma \delta l \frac{M^2}{l^2}
\end{eqnarray}
...
%\subsubsection{Deriving Eq (\ref{fin-result})}
%
