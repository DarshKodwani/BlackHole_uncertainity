...
\footnote{In the geometric limit, the shift in geodesics faithfully represents the shift in wave modes. Away from the geometric limit, the relation between them is more complicated, but one being large still implies that the other is, too.}
...
\end{enumerate}
%If we were reporting a positive result, that such uncertainty upholds unitarity, then one should question the validaty of learning a general lesson from a special example. 
%However, we will show that this particular form of $\Delta g_{\mu\nu}$ is already sufficient to question unitarity, and we did not choose this form to specifically do that. 
%Thus our limited analysis is sufficient to raise a reasonable doubt to the classical-background approximation.
...
After the non-unitary propagation, such a informationless mode has to become an asymptotic Hawking quantum which carries the information to purify the rest of the Hawking radiation there.
The exact detail of this process remains an enigma.
...
More precisely, the particular effect of local geometric uncertainty here is to create extra shifts between parallel null geodesics, which is reminiscent of memory effects or supertranslation\cite{GW_memory,HeLys14,KodPen16}.
This naturally echoes the idea that the soft hairs \cite{HawPer16} might encode the black hole information and returns them to the Hawking radiation via an interaction that is beyond the na\"ive local QFT\cite{OsuPag16}. 
